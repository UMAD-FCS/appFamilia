...
server <- function(input, output, session) {  
   ...
   output$selectcorte1 <- renderUI({
     selectInput("corte1", "Resultados por:", choices = unique(df$CORTE1[df$NOMINDICADOR ==input$indicador_CE_tp]))
     })
  
   #output$selectcorte1 <- renderUI({
   #     selectInput("corte1", "Resultados por:", choices = base_CE_tp()  %>% pull(CORTE1) %>% unique())
   #  })
  
  
   #selected_indicator <- reactive({
   #    df %>% filter(NOMINDICADOR == input$indicador_CE_tp)
   #  })
  
#      output$anio1 <- renderUI({
#        indicator_data <- selected_indicator()
#        if (any(indicator_data$CORTE2 != "NA")) {
#          selectInput(
#            "anio_select",
#            "Seleccione el año:",
#            choices = unique(indicator_data$FECHA)
#          )
#        }
#        else (any(indicator_data$CORTE2 == "NA")) {
#          
#        }
#      })
